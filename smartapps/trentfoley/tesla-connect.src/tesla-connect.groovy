...
def loginToTesla() {
	...
	return dynamicPage(name: "loginToTesla", title: "Connect your Tesla", nextPage:"selectVehicles", uninstall:showUninstall) {
    	section("To generate an access token use an app like \"Auth App for Tesla\" on iOS, or \"Tesla Tokens\" on Android.") {}
		section("Provide your Tesla tokens:") {
			input "accessToken", "text", title: "Access Token", required: true, autoCorrect: false
            input "refreshToken", "text", title: "Refresh Token", required: false, autoCorrect: false
		}
		section("To use Tesla, SmartThings encrypts and securely stores your Tesla tokens.") {}
	}
}
...
def getAuthServerUrl() { "https://auth.tesla.com" }
def getClientId () { "ownerapi" }
